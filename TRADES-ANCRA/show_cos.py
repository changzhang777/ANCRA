...
parser.add_argument('--model-dir', default='./checkpoint/baseline/PGD-AT_128_resnet18_cifar10',
...
def main():
    ...
    method = (args.model_dir.split('/')[-1]).split('_')[0]
    """torch.save(PPpcos, '../openi/three/%d-%s-PPpcos.npy' % (args.target_label, method))
    torch.save(PPpL2, '../openi/three/%d-%s-PPpL2.npy' % (args.target_label, method))
    torch.save(NPpcos, '../openi/three/%d-%s-NPpcos.npy' % (args.target_label, method))
    torch.save(NPpL2, '../openi/three/%d-%s-NPpL2.npy' % (args.target_label, method))"""
    torch.save(PPfcos, '../openi/three/%d-%s-PPfcos.npy' % (args.target_label, method))
    torch.save(PPfL2, '../openi/three/%d-%s-PPfL2.npy' % (args.target_label, method))
    torch.save(NPfcos, '../openi/three/%d-%s-NPfcos.npy' % (args.target_label, method))
    torch.save(NPfL2, '../openi/three/%d-%s-NPfL2.npy' % (args.target_label, method))
