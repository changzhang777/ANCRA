...
parser.add_argument('--dataset', type=str,
                    help='cifar10,cifar100,tiny-imagenet', default='cifar100')
parser.add_argument('--model-dir', default='./checkpoint/baseline/Mart_128_resnet18_cifar100/',
...
def main():
    ...
    testset = torchvision.datasets.CIFAR100(root='../../data', train=False, download=True, transform=trans_test)
    test_loader = torch.utils.data.DataLoader(testset, batch_size=args.batch_size, shuffle=False, drop_last=False,
                                              num_workers=2, pin_memory=True)

    for path in ['model_120.pth']:#, 'model_120.pth' model-res-epoch120.pt
        model_path = os.path.join(args.model_dir, path)
        print(model_path)
        logger.info(model_path)
        model = resnet.ResNet18(100).cuda()
        if 'best' in path:
