...
parser.add_argument('--model-dir', default='./model/ANCRA-WideResNet34-TRADES',
...
parser.add_argument('--ckpt_url', default="72/model-wrn-last.pt", help='pretrain model path')
        ...
        for _ in range(perturb_steps):
            ...
            with torch.enable_grad():
                ...
                for i in range(len(extra_output_adv)):
                    channel_reg_loss += F.cross_entropy(extra_output_adv[i], y)
...
def trades_loss_with_ANC(model,
                         x_natural,
                         y,
                         data_neg,
                         neg_gt,
    ...
    output_t, out_t_extra = model(data_neg, neg_gt, _eval=False)
    loss_robust_t = (1.0 / batch_size) * loss_fn(F.softmax(logits, dim=1), F.softmax(output_t, dim=1))
    extra_loss_robust_t = 0.
    for i in range(len(out_t_extra)):
        extra_loss_robust_t += (1.0 / batch_size) * loss_fn(F.softmax(extra_output_nat[i], dim=1),
                                                            F.softmax(out_t_extra[i], dim=1))
    extra_loss_robust_t /= len(out_t_extra)
    loss_robust_t += cr_beta * extra_loss_robust_t
...
def pick_neg(args, model, data, label, epoch):
    ...
    if 'random' in args.strategy or epoch <= 10:
        # randomly choose negative sample from the current batch
        ...
        for i in range(soft_label.shape[0]):
            ...
            if neg_id_list == []:
                neg_class_l = [random.sample(
                    [num for num in range(10) if (num != label[i].item()) and (len(class_to_id[num]) >= sn)], scn)]
                neg_class_l = list(itertools.chain.from_iterable(neg_class_l))
...
def TRADES_train(args, model, train_loader, optimizer, epoch):
    ...
    for batch_idx, (data, label) in enumerate(train_loader):
        ...
        optimizer.zero_grad()
        # calculate robust loss
        loss, loss_natural, loss_robust, loss_robust_t, logits, output_adv, output_t = \
            trades_loss_with_ANC(model=model,
                                 x_natural=data,
                                 y=target,
                                 data_neg=data_neg,
                                 neg_gt=neg_gt,
        ...
        an_l2 = torch.linalg.norm(
            logits.unsqueeze(1).expand(target.shape[0], scn * sn, 10).resize(scn * sn * target.shape[0], 10)
            - output_t).float() / ((scn * sn * target.shape[0]) ** 0.5)

        # print progress
        if batch_idx % args.log_interval == 0:
            print(
                'Train Epoch: {} [{}/{} ({:.0f}%)]\tLoss: {:.6f}, class_loss: {:.4f}, adv_loss: {:.4f}, adv_t_loss: {:.4f}, Nat_Correct: {:.4f}, Adv_Correct_Untarget: {:.4f}, Adv_Correct_target: {:.4f}, ap_l2: {:.2f}, an_l2: {:.2f}'.format(
                    epoch, batch_idx * len(data), len(train_loader.dataset),
                           100. * batch_idx / len(train_loader), loss.item(), loss_natural, loss_robust, loss_robust_t,
                           100 * nat_correct, 100 * adv_correct_u, 100 * adv_correct_t, ap_l2, an_l2))
            log_writer.write(
                'Train Epoch: {} [{}/{} ({:.0f}%)]\tLoss: {:.6f}, class_loss: {:.4f}, adv_loss: {:.4f}, adv_t_loss: {:.4f}, Nat_Correct: {:.4f}, Adv_Correct_Untarget: {:.4f}, Adv_Correct_target: {:.4f}, ap_l2: {:.2f}, an_l2: {:.2f}\n'.format(
                    epoch, batch_idx * len(data), len(train_loader.dataset),
                           100. * batch_idx / len(train_loader), loss.item(), loss_natural, loss_robust, loss_robust_t,
                           100 * nat_correct, 100 * adv_correct_u, 100 * adv_correct_t, ap_l2, an_l2))
...
def eval_test(model, test_loader):
    ...
    with torch.no_grad():
        for data, target in test_loader:
            ...
            output, extra_output = model(data, _eval=True)
            ...
            output_adv, extra_output_adv = model(adv_data, _eval=True)
            ...
            extra_loss_adv = 0.
            for output_adv in extra_output_adv:
                extra_loss_adv += F.cross_entropy(output_adv, target, size_average=False).item()
            extra_loss_adv /= len(extra_output_adv)
            test_loss_adv += args.cr_beta * extra_loss_adv
    """print('after test:')
    print(model.module.bn1.running_mean, model.module.bn1.running_var)"""
...
def main():
    ...
    log_writer.write(' alpha ' + str(args.alpha) + ' zeta ' + str(args.zeta) + ' beta ' + str(args.beta) + ' lr ' + str(
        args.lr) + ' weight_decay ' + str(args.weight_decay) + ' epochs ' + str(args.epochs) + '\n')
    model = wideresnet.WideResNet_RA().cuda()
    ...
    for epoch in range(args.restart_epoch, args.epochs + 1):
        ...
        # train
        Loss, Class_loss, Adv_loss, Adv_t_loss, Nat_correct, Adv_correct_u, Adv_correct_t, Ap_l2, An_l2 = \
            TRADES_train(args, model, train_loader, optimizer, epoch)
        """print('after train:')
        print(model.module.bn1.running_mean, model.module.bn1.running_var)"""

        print(
            'Train Epoch: {}, Loss: {:.6f}, NAT_Loss: {:.6f}, ADV_Loss: {:.6f}, ADV_T_Loss: {:.6f}, Nat_Correct: {:.4f}, Adv_Correct_Untarget: {:.4f}, Adv_Correct_target: {:.4f}, AP_l2: {:.6f}, AN_l2: {:.6f}'.format(
                epoch, Loss, Class_loss, Adv_loss, Adv_t_loss, Nat_correct, Adv_correct_u, Adv_correct_t, Ap_l2, An_l2))
        ...
        torch.save(model.module.state_dict(),
                   os.path.join(model_dir, 'model-wrn-last.pt'))
