...
parser.add_argument('--model-dir', default='./model/ANCRA-PreActResNet18-TRADES',
...
def pick_neg(args, model, data, label, epoch):
    ...
    if 'random' in args.strategy or epoch <= 10:
        neg_class_l = [random.sample([num for num in range(class_number) if (num != lb) and (len(class_to_id[num]) >= sn)], scn)
                       for lb in label]
        ...
        for i in range(soft_label.shape[0]):
            ...
            if neg_id_list == []:
                neg_class_l = [random.sample(
                    [num for num in range(class_number) if (num != label[i].item()) and (len(class_to_id[num]) >= sn)], scn)]
                neg_class_l = list(itertools.chain.from_iterable(neg_class_l))
...
def trades_train(args, model, train_loader, optimizer, epoch):
    ...
    for batch_idx, (data, label) in enumerate(train_loader):
        ...
        if batch_idx % args.log_interval == 0:
            print(
                'Train Epoch: {} [{}/{} ({:.0f}%)]\tLoss: {:.6f}, class_loss: {:.4f}, adv_loss: {:.4f}, adv_t_loss: {:.4f}'.format(#, Nat_Correct: {:.4f}, Adv_Correct_Untarget: {:.4f}, Adv_Correct_target: {:.4f}, ap_l2: {:.2f}, an_l2: {:.2f}
...
def main():
    ...
    train_loader = torch.utils.data.DataLoader(trainset, batch_size=args.batch_size, shuffle=True, drop_last=False,
                                               num_workers=4)
    test_loader = torch.utils.data.DataLoader(testset, batch_size=args.test_batch_size, shuffle=False, drop_last=False,
                                              num_workers=4)

    model = preactresnet.PreActResNet18_RA(num_classes=class_number).cuda()
    ...
    for epoch in range(args.restart_epoch, args.epochs + 1):
        ...
        print(
            'Train Epoch: {}, Loss: {:.6f}, NAT_Loss: {:.6f}, ADV_Loss: {:.6f}, ADV_T_Loss: {:.6f}'.format(#, Nat_Correct: {:.4f}, Adv_Correct_Untarget: {:.4f}, Adv_Correct_target: {:.4f}, AP_l2: {:.6f}, AN_l2: {:.6f}'.format(
        ...
        torch.save(save_state,
                   os.path.join(model_dir, 'last_state.zip'))
