...
class ANCRA_BasicBlock(nn.Module):
    ...
    def forward(self, x, y, fc, bn1, relu, nChannels):
        ...
        fc_in = F.avg_pool2d(out, 8)
        fc_in = fc_in.view(-1, nChannels)
        fc_out = fc(fc_in)
...
class NetworkBlock_ANCRA(nn.Module):
    def __init__(self, nb_layers, in_planes, out_planes, block, stride, dropRate=0.0):
        super(NetworkBlock_ANCRA, self).__init__()
...
class WideResNet_RA(nn.Module):
    def __init__(self, depth=34, num_classes=10, widen_factor=10, dropRate=0.0):
        ...
        self.block2 = NetworkBlock(n, nChannels[1], nChannels[2], block, 2, dropRate)
        # 3rd block
        self.block3 = NetworkBlock_ANCRA(n, nChannels[2], nChannels[3], block, 2, dropRate)
        # global average pooling and classifier
        self.bn1 = nn.BatchNorm2d(nChannels[3])
        self.relu = nn.ReLU(inplace=True)
        self.fc = nn.Linear(nChannels[3], num_classes)
    ...
    def forward(self, x, y=None, _eval=None):
        ...
        out = self.block1(out)
        out = self.block2(out)
        out, fc_out = self.block3(out, y, self.fc, self.bn1, self.relu, self.nChannels)
        out = F.avg_pool2d(out, 8)
        out = out.view(-1, self.nChannels)
        if _eval==None:
            return self.fc(out)
        else:
            return self.fc(out), fc_out
